...
import Lunarflow.Renderer.WithHeight (YLayoutLikeF, YMeasures, YLayoutLike, totalHeight)
...
render (Tuple rootMeasures layout) =
  ...
  where
  algebra :: GAlgebra _ (YLayoutLikeF v c a l) (RenderM RenderList)
  algebra (Var { position, inScope, name, color, freeTerms }) =
    withFreeVars freeTerms do
      { xStart } <- ask
  ...
  algebra (Lambda data'@{ args, heights, position, isRoot, inScope, scope } (Tuple _ body)) = do
    xStart <- ask <#> _.xStart
    yOffset <- getYOffset inScope
  ...
  algebra (Call { position, inScope, freeTerms } (Tuple functionExpression mkFunc) (Tuple _ mkArg)) =
    withFreeVars freeTerms do
      -- Get stuff from the environment
      yOffset <- getYOffset inScope
      --
      rawArgument <- mkArg
      function <- local _ { xStart = rawArgument.maxX } mkFunc
      let
        argumentContinuation =
          Shape.rect { fill: rawArgument.color }
            { x: rawArgument.maxX
            , width: function.maxX - rawArgument.maxX
            , height: lineHeight
            , y: rawArgument.lineY
            }
        ...
        argument =
          rawArgument
            { maxX = function.maxX
            , shapes = Array.cons argumentContinuation rawArgument.shapes
            }
...
withFreeVars :: forall a. HashMap.HashMap String a -> RenderM ~> RenderM
withFreeVars vars m = do
  { xStart } <- ask
  flip local m \ctx ->
    ctx
      { starts = ctx.starts <> NameMap.fromFree (xStart <$ vars)
      }
